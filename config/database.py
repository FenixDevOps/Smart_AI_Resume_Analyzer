...
def delete_resume(resume_id):
    ...
    cursor.execute(
        "DELETE FROM resume_data WHERE id = ?",
        (resume_id,)
    )
...
def verify_admin(email, password):
    ...
    try:
        cursor.execute('SELECT * FROM admin WHERE email = ? AND password = ?', (email.strip(), password.strip()))
        result = cursor.fetchone()
        print("Login Debug:",email,password,result)
